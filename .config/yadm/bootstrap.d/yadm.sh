...
if ! type -P yadm &> /dev/null; then
  ...
  ln -sfv "$(realpath "${LIB_HOME}"/yadm/yadm)" "${BIN_HOME}/"

  log_info "Updating manpages..."
  ln -sfv "$(realpath "${LIB_HOME}"/yadm/yadm.1)" "${MAN_HOME}/man1/"

  log_info "Updating fish completions..."
  ln -sfv "$(realpath "${LIB_HOME}"/yadm/completion/fish/yadm.fish)" "$FISH_COMPLETIONS_DIR/"
fi
