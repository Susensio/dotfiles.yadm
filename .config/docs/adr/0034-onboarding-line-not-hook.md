# ADR-0034: Onboard projects with a CLAUDE.md line instead of a session-start hook
...
Onboarding is one bullet in `CLAUDE.md`: read what a project says about itself before exploring it, find where it declares how it is run and tested, drive it through that interface.
...
A hook fires whether or not the agent was paying attention; a `CLAUDE.md` bullet is read and may be skipped, and it reaches subagents only because the whole `CLAUDE.md` hierarchy does.
...
A trivial session in a directory with no project pays a few tokens for guidance that will not fire, which is the same bargain the other `CLAUDE.md` bullets already make.
