...
Harness material had accumulated in three places -- this repo, a generator repo (`antigravity-harness`), and a consuming project -- with no rule deciding what belonged where.
The result was three copies of the `adr` skill, three of `git-commit`, and two agents both named `tester` doing different jobs.
Some of that divergence was correct: `git-commit` disagreed about conventional-commit prefixes because the repos genuinely disagree.
The defect was not the difference but that it was maintained by copying.

Separately, ADR-0023 kept the delegation and model-selection policy in `CLAUDE.md`, arguing that trigger-scoped rules are inert for a reader they do not apply to.
That reasoning weighed `CLAUDE.md` against an extracted agent role and did not consider the third option.
Context files, `.claude/rules/` and imports reach every subagent with no opt-out -- only built-in `Explore` and `Plan` skip them, and that is not configurable.
A skill costs its `description` until it fires.
Seven lines of delegation policy were therefore shipping into every spawn, including agents that never delegate.

## Decision

Two tiers, one placement question: **would this sentence be false in another project?**
Yes places it in the project tier, no in the user tier (`.config/agents/`, symlinked into `~/.claude/`).
...
One copy of each generic skill, at the tier where it is true.
