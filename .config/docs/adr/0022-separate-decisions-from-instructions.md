...
`environment-architecture.md` is the instance that exists, and covers environment-variable flow only.
...
The boundary between "what it is" (`environment-architecture.md`) and "why" (ADRs) needs active maintenance, or the two drift into restating each other.
The rule only holds if the agent applying it checks — nothing enforces the routing mechanically.

## Corrections

2026-08-25: the routing line named `environment-architecture.md` as the destination for "what the setup is now".
