...
The rule as current practice lives in `agents/skills/audit-harness/design-rules.md` as R16, enforced by a check in the audit beside it, with the always-loaded instruction in `GLOBAL.md`.
...
Being a user-tier rule, R16 yields to a repo whose tracked markdown already holds to another convention.
