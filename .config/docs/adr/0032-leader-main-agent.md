...
ADR-0026 then superseded it on placement, moving the delegation policy into a skill; ADR-0030 moved the trigger back to `CLAUDE.md`.
...
The pronoun test made the boundary visible rather than arguable -- `CLAUDE.md`'s personally-worded bullets ("reporting to me", "the size I asked for", "let me pick") were, without exception, the main-session-only ones, while every impersonal bullet was genuinely universal.
...
Against extraction: it adds a layer to maintain, it applies to a weekend spike as much as a mature repository, and the token saving is close to nothing, because the delegation trigger has to stay in `CLAUDE.md` regardless.
...
Content that was main-session-only moved out of `CLAUDE.md` accordingly: commits to `leader`, agent-memory routing to the `project-record` skill, and concision toward the user to the `outputStyle` setting, which reaches the main conversation and not subagents.
...
`CLAUDE.md` drops from 25 lines to 12, and what left it stops reaching every spawn.
...
- `documenter` receives two `CLAUDE.md` bullets it cannot act on, holding neither `Bash` nor web tools.
