...
## R-harness-map. `docs/harness.md` describes the global userspace harness; whoever changes the harness updates it.
...
It is the one context file a person reads to onboard into the global `~/.config/claude/`; keeping it current is the editor's responsibility, not the reader's, because a stale map is worse than none -- it reads as authoritative and is not.
Note that this doc describes the global userspace harness applied across all projects, not a project-local harness (like `.claude/`).
Skip the doc edit only where the change is invisible from the doc's altitude: rewording a skill's internal instructions, not what it is for or when it fires.

*Failure it prevents:* `docs/harness.md` reading as a local project's config rather than the global userspace setup, or naming an agent already deleted, or silent about one that has been running for months -- trusted by the next reader precisely because nothing marked it stale.
