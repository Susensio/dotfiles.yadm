...
map("n", "[d", function()
   ...
   local severities = vim.tbl_map(function(item) return item.severity end, diagnostics)
   local max_severity = vim.fn.min(severities)
   vim.diagnostic.goto_prev({ wrap = false, severity = max_severity })
   require("utils.loclist").follow_cursor()
end,
...
map("n", "]d", function()
   ...
   local severities = vim.tbl_map(function(item) return item.severity end, diagnostics)
   local max_severity = vim.fn.min(severities)
   vim.diagnostic.goto_next({ wrap = false, severity = max_severity })
   require("utils.loclist").follow_cursor()
end,
